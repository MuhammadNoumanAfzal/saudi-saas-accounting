...
export function QuotationCreateSheet({
  open,
  onClose,
  onOpenChange,
  quotationId,
  ...
  const [terms, setTerms] = useState(
    t('Prices valid for 30 days. 15% VAT included where applicable.', 'الأسعار سارية لمدة 30 يوماً. تشمل ضريبة القيمة المضافة 15٪ عند الانطباق.')
  );
  ...
  useEffect(() => {
    if (initialData) {
      ...
      setValidUntilDate(initialData.validUntilDate ? new Date(initialData.validUntilDate).toISOString().split('T')[0] : new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0]);
  ...
  const updateItemRow = (index: number, field: keyof QuotationItemInput, value: any) => {
    setItems((prev) => {
      const copy = [...prev];
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (quotationId) {
      updateMutation.mutate(
        { organizationId: orgId, quotationId, data: payload },
        {
          onSuccess: () => {
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <SheetTitle className="text-xl font-bold text-foreground">
                {quotationId ? t('Edit Quotation', 'تعديل عرض السعر') : t('New Quotation', 'عرض سعر جديد')}
              </SheetTitle>
              <SheetDescription className="text-xs text-muted-foreground mt-0.5">
                {t(
                  'Create a ZATCA-compliant sales estimate or formal quotation for a customer.',
                  'إنشاء عرض سعر أو تقدير تكلفة معتمد ومطابق لمتطلبات هيئة الزكاة والضريبة.'
                )}
              </SheetDescription>
            </div>
          </div>
