...
export function PartyCreateSheet({ 
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (type === 'organization') {
      if (!nameEn) newErrors.nameEn = t('Business name is required', 'اسم المنشأة مطلوب');
    } else {
      if (!firstName) newErrors.firstName = t('First name is required', 'الاسم الأول مطلوب');
    }
    
    if (vatRegistered) {
      if (!vatNumber) newErrors.vatNumber = t('VAT number is required', 'الرقم الضريبي مطلوب');
      else if (!/^\d{15}$/.test(vatNumber)) newErrors.vatNumber = t('Format must be 15 digits', 'يجب أن يتكون من 15 رقماً');
    }
    ...
    const payload = {
      partyType: type,
      businessNameEnglish: type === 'organization' ? nameEn : null,
      businessNameArabic: type === 'organization' ? nameAr : null,
      ...
      city: city || null,
      legalNameEnglish: legalEn || null,
      legalNameArabic: legalAr || null,
