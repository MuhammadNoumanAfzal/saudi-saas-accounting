...
export function PartyImportSheet({ 
  ...
  const handleDownloadTemplate = async () => {
    try {
      const blob = await downloadPartyImportTemplate(orgId, role as any);
    ...
    <Sheet open={open} onOpenChange={handleOpenChange}>
      <SheetContent side={isRtl ? 'left' : 'right'} className="w-full sm:max-w-md md:max-w-lg p-0 flex flex-col bg-background">
        <SheetHeader className="p-6 border-b border-border bg-card/50">
          ...
          <SheetDescription>
            {t('Add multiple records at once using a CSV file.', 'إضافة عدة سجلات دفعة واحدة باستخدام ملف CSV.')}
          </SheetDescription>
        </SheetHeader>
