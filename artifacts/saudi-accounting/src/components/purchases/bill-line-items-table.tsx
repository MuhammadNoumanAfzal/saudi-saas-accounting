...
interface BillLineItemsTableProps {
  ...
  updateItemRow: (index: number, field: keyof BillItemInput, val: any) => void;
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between border-b border-border/80 pb-2">
        <h3 className="font-bold text-sm text-foreground flex items-center gap-2">
          ...
          <span>{t('Purchase Line Items & Services', 'بنود فاتورة الشراء والخدمات')} <span className="text-red-500">*</span></span>
        </h3>
      ...
      <div className="space-y-3">
        {items.map((item, idx) => (
          <div key={idx} className={`p-4 rounded-xl border ${errors[`item_${idx}_desc`] || errors[`item_${idx}_qty`] || errors[`item_${idx}_price`] ? 'border-red-500 bg-red-500/5' : 'border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900'} shadow-2xs space-y-3 relative group`}>
            <div className="grid grid-cols-1 md:grid-cols-12 gap-3 items-start">
              ...
              <div className="md:col-span-4 space-y-1">
                ...
                <select
                  className="w-full px-2.5 py-1.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-xs font-medium text-foreground focus:outline-none focus:ring-2 focus:ring-indigo-500/20"
              ...
              <div className="md:col-span-7 space-y-1">
                ...
                <input
                  className={`w-full px-2.5 py-1.5 bg-slate-50 dark:bg-slate-800 border ${errors[`item_${idx}_desc`] ? 'border-red-500 bg-red-500/5' : 'border-slate-200 dark:border-slate-700'} rounded-lg text-xs text-foreground focus:outline-none focus:ring-2 focus:ring-indigo-500/20`}
                  placeholder={t('Item description', 'وصف المشتريات أو الخدمة')}
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 pt-2 border-t border-slate-200 dark:border-slate-800">
              <div>
                ...
                <input
                  ...
                  step="0.0001"
                  min="0.0001"
                  className={`w-full px-2.5 py-1.5 bg-slate-50 dark:bg-slate-800 border ${errors[`item_${idx}_qty`] ? 'border-red-500 bg-red-500/5' : 'border-slate-200 dark:border-slate-700'} rounded-lg text-xs font-mono text-foreground focus:outline-none focus:ring-2 focus:ring-indigo-500/20`}
                  value={item.quantity}
              ...
              <div>
                ...
                <input
                  ...
                  step="0.01"
                  min="0"
                  className={`w-full px-2.5 py-1.5 bg-slate-50 dark:bg-slate-800 border ${errors[`item_${idx}_price`] ? 'border-red-500 bg-red-500/5' : 'border-slate-200 dark:border-slate-700'} rounded-lg text-xs font-mono text-foreground focus:outline-none focus:ring-2 focus:ring-indigo-500/20`}
              ...
              <div>
                ...
                <div className="w-full px-2.5 py-1.5 bg-slate-100 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 rounded-lg text-xs font-mono font-bold text-foreground flex items-center justify-end">
                  {(
                    Math.max(
                      0,
